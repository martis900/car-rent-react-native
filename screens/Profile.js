...
import { Platform, View, Text, Button, TouchableOpacity, FlatList, ScrollView, Alert, Dimensions, Image } from 'react-native'
...
const HotDeals = ({ navigation }) => {
    return (
        <FlatList
            ...
            renderItem={({ item }) => (
                <View kye={item.id} style={{ paddingLeft: 15, height: 230 }}>
                    <Shadow style={{ display: 'flex', paddingTop: 10, height: 225, width: 180, borderRadius: 15, backgroundColor: 'rgb(21, 76,196)' }}>
                        ...
                        <View style={{ display: 'flex', paddingLeft: 20, height: 137, }} >
...
Profile.navigationOptions = {
    header: (
        <View style={{ display: 'flex', flexDirection: 'row', backgroundColor: 'white', justifyContent: 'space-between', height: Platform.OS === 'ios' ? 65 : 45, alignItems: 'center', paddingTop: Platform.OS === 'ios' ? 35 : 15 }}>
