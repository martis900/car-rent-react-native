    ...
    static navigationOptions = {
        header: (
            <View style={{ display: 'flex', flexDirection: 'row', backgroundColor: 'rgb(21, 76,196)', justifyContent: 'space-between', height: Platform.OS === 'ios' ? 40 : 30, alignItems: 'center', paddingTop: Platform.OS === 'ios' ? 40 : 20 }}>
                <Icon style={{ paddingLeft: 25 }} name="ios-arrow-round-back" color='#ffffff' size={48} onPress={() => this.props.navigation.goBack()}></Icon>
    ...
    render() {
        return (
            <View style={{ display: 'flex', flexDirection: 'column', flex: 1, backgroundColor: 'rgb(21, 76,196)' }}>
                <View style={{ display: 'flex', alignItems: 'center', paddingTop: 5, scaleY: this.props.arrowUp ? 1 : -1 }}>
                    <Icon name="ios-arrow-up" color='white' size={20} />
                </View>
                <View style={{ display: 'flex', flexDirection: 'row', paddingLeft: 15 }}>
                    <Text style={{ fontWeight: 'bold', fontSize: 36, color: '#ffffff' }}>{HotDealsList.HotDeals[2 - 1].name} </Text>
                    <Text style={{ paddingTop: 7, fontWeight: "normal", fontSize: 28, color: '#ffffff' }}>{HotDealsList.HotDeals[2 - 1].model}</Text>
                </View>

                <View style={{ display: 'flex', flex: 2, justifyContent: 'center' }}>
                    <SliderBox style={{ height: 10, backgroundColor: 'red', alignSelf: 'center' }} images={HotDealsList.HotDeals[2 - 1].img} dotColor="#ffffff" inactiveDotColor="rgb(128,160,226)" paginationBoxVerticalPadding={10} dotStyle={{ width: 8, height: 8 }} />
                </View>
                ...
                <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: "center", flex: 1 }}>
                    <Text style={{ color: '#ffffff', fontSize: 18, alignSelf: 'center', marginLeft: 40 }}>{HotDealsList.HotDeals[2 - 1].price}/day</Text>
                    <TouchableOpacity>
